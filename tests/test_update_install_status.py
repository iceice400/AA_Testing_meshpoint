...
from src.api.update.git_runner import GIT_BIN
...
class _FakeGitRunner:
    ...
    def __call__(
        ...
        if args[:3] == [GIT_BIN, "fetch", "origin"]:
            return 0, "", ""
        if args[:2] == ["sudo", "/bin/bash"] and "ensure_git_safe.sh" in args[2]:
            return 0, "", ""
...
class TestMatchChannelForBranch(unittest.TestCase):
    ...
    def test_field_all_features_maps_to_field_channel(self) -> None:
        info = match_channel_for_branch(ReleaseChannelRegistry(), "field/all-features")
        self.assertEqual(info["active_channel_id"], "field-all-features")
        self.assertEqual(info["channel_tier"], "experimental")
...
class TestBuildInstallStatusPayload(unittest.TestCase):
    ...
    def test_sync_reports_commits_behind(self) -> None:
        ...
        self.assertTrue(
            any(
                len(c) >= 3 and c[:3] == [GIT_BIN, "fetch", "origin"]
