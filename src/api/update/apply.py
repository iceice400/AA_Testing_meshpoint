...
class UpdateApplier:
    ...
    def apply(
        ...
        *,
        ...
        start = time.time()
        log: list[dict] = []
        ensure_git_safe_directory(self._repo_path, runner=self._runner)
    ...
    def rollback(
        ...
        *,
        ...
        steps = [
            ApplyAttempt(
                label="git reset",
                args=["sudo", GIT_BIN, "reset", "--hard", sha],
                cwd=self._repo_path,
            ),
            ApplyAttempt(
                label="upgrade",
    ...
    def _build_chain(self, branch: str) -> Iterable[ApplyAttempt]:
        return (
            ApplyAttempt(
                label="git fetch",
                args=["sudo", GIT_BIN, "fetch", "origin", branch],
                cwd=self._repo_path,
                timeout_seconds=180,
            ),
            ApplyAttempt(
                label="git checkout",
                args=["sudo", GIT_BIN, "checkout", "-f", branch],
                cwd=self._repo_path,
                timeout_seconds=60,
            ),
            ApplyAttempt(
                label="git reset",
                args=["sudo", GIT_BIN, "reset", "--hard", f"origin/{branch}"],
                cwd=self._repo_path,
                timeout_seconds=60,
            ),
            ApplyAttempt(
