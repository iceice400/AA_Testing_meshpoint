...
from src.api.update.git_runner import (
    ...
    git_prefix,
    shell_runner as default_git_runner,
)
...
def read_install_git_ref(
    ...
    *,
    ...
    """
    git = git_prefix(use_sudo=use_sudo)
...
def build_install_status_payload(
    *,
    ...
    if branch is None and sha is None and use_sudo:
        if ensure_git_safe_directory(repo_path, runner=runner):
            branch, sha = read_install_git_ref(
                repo_path, runner=runner, use_sudo=use_sudo,
            )
    if branch:
    ...
    if sync_remote and compare_branch:
        ensure_git_safe_directory(repo_path, runner=runner)
        ok, err = git_fetch_origin_branch(
            repo_path, compare_branch, runner=runner, use_sudo=use_sudo,
        )
        if not ok and err and "dubious ownership" in err.lower():
            if ensure_git_safe_directory(repo_path, runner=runner):
                ok, err = git_fetch_origin_branch(
                    repo_path, compare_branch, runner=runner, use_sudo=use_sudo,
                )
