...
def init_automation(automation_config: AutomationConfig) -> None:
    """Bind LAN automation token settings for ``/api/automation/*``."""
    global _automation_config
    _automation_config = automation_config


def reset_auth() -> None:
    """Test helper: clear module-level state between cases."""
    global _jwt_service, _automation_config
    _jwt_service = None
...
async def require_automation_auth(
    ...
    """Dependency: LAN automation token or valid dashboard JWT when enabled."""
    if _automation_config is None or not _automation_config.enabled:
        raise HTTPException(
            status_code=status.HTTP_403_FORBIDDEN,
            detail="automation API disabled",
        )

    claims = await optional_auth(request, authorization)
    if claims is not None:
        return claims

    expected = (_automation_config.token or "").strip()
    if not expected:
        raise HTTPException(
            status_code=status.HTTP_403_FORBIDDEN,
            detail="automation token not configured",
        )

    supplied = (request.headers.get("X-Meshpoint-Token") or "").strip()
    if not supplied and authorization and authorization.startswith(_BEARER_PREFIX):
