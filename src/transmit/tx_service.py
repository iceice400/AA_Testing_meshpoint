...
class TxService:
    ...
    async def send_traceroute_request(
        ...
        """Send a TRACEROUTE probe to a specific node (Tier B poller)."""
        if not self.meshtastic_enabled:
            return SendResult(success=False, protocol="meshtastic", error="TX unavailable")
        ...
        if dest_int == BROADCAST_ADDR_MT:
            return SendResult(
                success=False, protocol="meshtastic", error="Invalid traceroute destination"
            )
        ...
        if packet_bytes is None:
            return SendResult(
                success=False, protocol="meshtastic", error="Traceroute request build failed"
            )
