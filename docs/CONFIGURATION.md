...
Enable broker TLS/mqtts from **Configuration → MQTT** or in ``local.yaml``:

```yaml
mqtt:
  tls_enabled: true
  tls_ca_cert: ""   # optional PEM path for private CA; empty uses system trust
```

When ``tls_enabled`` is false (default), the publisher uses plain TCP. Port
**8883** alone does not enable encryption — set ``tls_enabled: true`` and use
your broker's TLS port.

- Public Meshtastic MQTT: `mqtt.meshtastic.org` on port **1883** (default, no TLS).
- Private TLS brokers: set ``tls_enabled`` and ``port`` to the broker's mqtts port.
