...
# ── 3b. Git safe.directory (root-owned tree, dubious-ownership fix) ─
if [ -f "${MESHPOINT_DIR}/scripts/ensure_git_safe.sh" ]; then
    /bin/bash "${MESHPOINT_DIR}/scripts/ensure_git_safe.sh" "${MESHPOINT_DIR}"
fi

# ── 3c. Root-owned bytecode caches (sudo python / compileall) ───────
find "${MESHPOINT_DIR}/src" -type d -name __pycache__ -user root \
